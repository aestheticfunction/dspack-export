/**
 * Validate an assembled document against the bundled dspack v0.2 JSON Schema.
...
const schemaV02 = JSON.parse(
  readFileSync(join(__dirname, '..', '..', 'src', 'emit', 'schema', 'dspack.v0.2.schema.json'), 'utf-8'),
);
...
export function validateDspack(doc: unknown): ValidationResult {
  ...
  const errors = (validateV02.errors as Array<{ instancePath?: string; message?: string }>).map(
